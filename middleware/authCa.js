...
async function checkCaToken(token) {
    const session = await resolveCaTokenSession(token);
    return session?.username || null;
}

async function resolveCaTokenSession(token) {
...
async function caProfileExists(country_code, mobile) {
...
async function listCaProfilesByPhone(country_code, mobile) {
    const cc = normalizeCountryCode(country_code);
    const mob = normalizeMobileDigits(mobile);
...
async function authCa(req, res, next) {
    ...
    req.ca_username = session.username;
    req.ca_session = session;
    next();
}
