...
router.post("/login/send-otp", async (req, res) => {
    ...
    try {
        ...
        const caProfile = await findActiveCaProfile(country_code, mobile);
        if (!caProfile) {
            return res.status(404).json({
                success: false,
                message: "CA not found or account is inactive.",
            });
        }
        ...
        console.error("CA LOGIN SEND OTP ERROR:", err);
    ...
    try {
        ...
        const [otpRows] = await conn.query(
            `SELECT id
             ...
             ORDER BY id DESC
             LIMIT 1`,
            [normalizedCountryCode, normalizedMobile, CA_OTP_TYPE, String(otp), "0"]
        );
...
router.post("/logout", authCa, async (req, res) => {
    try {
        ...
        console.error("CA LOGOUT ERROR:", err);
        return res.status(500).json({
            success: false,
