...
async function checkClientToken(token) {
    const session = await resolveClientTokenSession(token);
    return session?.username || null;
}

async function resolveClientTokenSession(token) {
...
async function clientProfileExists(country_code, mobile) {
...
async function listClientProfilesByPhone(country_code, mobile) {
    const cc = normalizeCountryCode(country_code);
    const mob = normalizeMobileDigits(mobile);
...
async function authClient(req, res, next) {
    ...
    req.client_username = session.username;
    req.client_session = session;
    next();
}
