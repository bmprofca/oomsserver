...
// Bump this any time the layout/theme code changes. Previously, sample PDFs
// were cached to disk forever — `ensureFormatSamplesExist()` only rebuilt
// them if a file was *missing*, so a layout fix never reached users because
// the old (buggy) file was still sitting there and "existed". Baking the
// version into the filename means a bump here invalidates every cached
// sample automatically, without needing to manually clear `media/format/`.
...
function filePathFor(columnKey, formatKey) {
    return path.join(dirForColumnKey(columnKey), `${formatKey}.${TEMPLATE_VERSION}.pdf`);
}
...
/**
 * Writes classic / compact / minimal / premium_* PDFs under `media/format/<column>/`
 * for every `invoice_formats` column.
 */
