CREATE TABLE IF NOT EXISTS `email_broadcast_schedules` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `schedule_id` varchar(50) NOT NULL,
  `branch_id` varchar(50) NOT NULL,
  `name` varchar(255) NOT NULL,
  `config_id` varchar(100) DEFAULT NULL,
  `template_id` varchar(100) NOT NULL,
  `template_name` varchar(255) DEFAULT NULL,
  `global_variables_json` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL CHECK (json_valid(`global_variables_json`)),
  `audience` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL CHECK (json_valid(`audience`)),
  `daily_limit` int(11) DEFAULT 1000,
  `schedule_type` enum('daily','weekly','monthly') NOT NULL,
  `schedule_config` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL CHECK (json_valid(`schedule_config`)),
  `timezone` varchar(64) NOT NULL DEFAULT 'Asia/Kolkata',
  `is_active` tinyint(4) NOT NULL DEFAULT 1,
  `last_run_at` datetime DEFAULT NULL,
  `last_run_key` varchar(32) DEFAULT NULL,
  `last_broadcast_id` varchar(100) DEFAULT NULL,
  `last_error` text DEFAULT NULL,
  `create_by` varchar(100) DEFAULT NULL,
  `modify_by` varchar(100) DEFAULT NULL,
  `create_date` datetime DEFAULT NULL,
  `modify_date` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_email_schedule_id` (`schedule_id`),
  KEY `idx_email_sched_branch_active` (`branch_id`, `is_active`),
  KEY `idx_email_sched_last_run_key` (`last_run_key`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `email_broadcast_schedule_runs` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `run_id` varchar(50) NOT NULL,
  `schedule_id` varchar(50) NOT NULL,
  `branch_id` varchar(50) NOT NULL,
  `status` enum('success','failed','skipped') NOT NULL,
  `broadcast_id` varchar(100) DEFAULT NULL,
  `recipients_count` int(11) NOT NULL DEFAULT 0,
  `message` text DEFAULT NULL,
  `run_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_email_run_id` (`run_id`),
  KEY `idx_email_schedule_run` (`schedule_id`, `run_at`),
  KEY `idx_email_branch_run` (`branch_id`, `run_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
