...
router.get("/list", validateAgentSession, async (req, res) => {
    try {
        ...
        console.error("AGENT TRANSACTION LIST ERROR:", error);
...
router.post("/generate-invoice", validateAgentSession, async (req, res) => {
    try {
        const branch_id = req.branch_id;
        const username = req.agent_username;
        ...
        console.error("AGENT INVOICE GENERATE ERROR:", error);
        return res.status(500).json({
            success: false,
